...
height_of_ap   = 1.5; % Same is the height of UE
...
function plot_BF_pattern(BF,h)
    ...
    PhiGrid = zeros(size(azimGrid));
    ThetaGrid = zeros(size(azimGrid));
    for i =1:size(azimGrid,1)
        for j =1:size(azimGrid,2)
            PhiTheta = azel2phitheta([azimGrid(i,j);elevGrid(i,j)]);
            PhiGrid(i,j) = PhiTheta(1);
            ThetaGrid(i,j) = PhiTheta(2);
        end
    end
    [X, Y, Z]   = sph2cart(PhiGrid,ThetaGrid, magnification_factor*norm_BF);
    colormap turbo

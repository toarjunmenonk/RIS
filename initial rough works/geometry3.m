...
relected_elev = (pi/180)*60; % It should vary from -pi/2 to +pi/2. +ve value means RIS is below the level of UE
...
text(x_source,y_source,height_of_ue,'UE');hold on;
...
% Psi_suboptimum_1bit = [pi;0;pi;0];
% Psi_suboptimum_1bit = repelem(Psi_suboptimum_1bit ,16);
% Psi_suboptimum_1bit = repmat(Psi_suboptimum_1bit,[4,1]);
% temp = reshape(Psi_suboptimum_1bit,[16,16]);
% % temp = temp.';
% Psi_suboptimum_1bit = temp;
% Psi_suboptimum_1bit = Psi_suboptimum_1bit(:);
% figure;
% imagesc(temp)

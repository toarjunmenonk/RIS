...
reflected_azim = -(pi/180)*(50);% It should vary from 0 to -pi/2. 0 = partition wall, -pi/2 is -y axis
reflected_elev = (pi/180)*(-40); % It should vary from -pi/2 to +pi/2. +ve value means RIS is below the level of UE
...
Psi_proposed = eig_direction_azim*(sin(reflected_azim)*cos(reflected_elev)+sin(impinging_azim)*cos(impinging_elev))      + ...
               eig_direction_elev*(sin(reflected_elev)+sin(impinging_elev));
...
switch(choice_RIS_res)
    ...
    case 3
        ...
        res = " | Optimum";
    case 4
        BF = BF_pattern(nH,Psi_proposed,h_AP2RIS);
        plot_BF_pattern(BF,height_of_ris);
    ...
    hold on;
    srcImg = imrotate(imread("protractor.png"),-90);
    [xx, yy] =meshgrid(0:4, -4:4);
    zz = zeros(size(xx));  % set the z value
    w = warp(xx, yy, zz, srcImg);
    w.FaceAlpha = 0.5;
    hold on;

    [xx, zz] =meshgrid(0:2, -2:2);
    zz = zz+height_of_ris;
    yy = zeros(size(xx));  % set the z value
    zz(zz<0) = 0;
    w = warp(xx, yy, zz, srcImg);
    w.FaceAlpha = 0.5;
    hold on;    
...
function [eig_direction_azim,eig_direction_elev] = gen_eigen_dir(Psi_optimum,reflected_azim,reflected_elev,impinging_azim, impinging_elev)
    ...
    eig_direction_azim = Psi_optimum/(sin(reflected_azim+ impinging_azim)*cos(reflected_elev));
    eig_direction_elev = 1;
    % theta_centralised  = reflected_azim+impinging_azim;
    % phi_centralised    = reflected_elev-impinging_elev;
    % eig_direction_azim = Psi_optimum/(sin(theta_centralised)*cos(phi_centralised));
    % eig_direction_elev = repmat(Psi_optimum(1:16)/sin(phi_centralised),[16,1]);
    % eig_direction_azim = eig_direction_azim/(sin(impinging_azim)*cos(impinging_elev));
